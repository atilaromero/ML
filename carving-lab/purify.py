...
if __name__ == '__main__':
    folder = '../datasets/govdocs1/sample200/'
    all_files = block_sampler.files_from(folder)
    rnd_files = block_sampler.files_from('../datasets/random')
    categories = {}
    for f in all_files:
    ...
    for k in categories:
        outdir = '../datasets/purified/%s'%k
        if os.path.exists(outdir):
            continue
        os.makedirs(outdir)
        cat = categories[k]
        mix = cat.union(random.sample(rnd_files, len(cat)))
        ...
        model = main(config, tset, vset)
        for sample in categories[k]:
            outpath = os.path.join(outdir, os.path.basename(sample))
                ...
                with open(outpath, 'wb') as outfile:
                    while(True):
                        ...
                        for i in range(0,len(b),512):
                            ...
                            bs.append(blk)
                        ys = model.predict(block_sampler.xs_encoder_8bits_11(bs))
