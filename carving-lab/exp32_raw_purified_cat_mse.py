...
"""
Train 2 linked models, first with categorical_crossentropy, then with mse.

Poor results, would be better to remain with categorical_crossentropy only.
"""

def mk_result_dir():
    time_dir = datetime.datetime.now().isoformat()[:19].replace(':', '-')
    model_dir = os.path.join('results', 'exp32', time_dir)
...
def raw_purified(
    ...
    for dataset, name in [(pdataset, 'purified'), (rdataset, 'raw')]:
        ...
        modelcat, modelmse = models.double_models(n, 8)
        mymodels[name] = modelmse

        timeIt = callbacks.TimeIt()

        modelcat.fit_generator(iter(tbenc),
        ...
        modelmse.fit_generator(iter(tbenc),
            validation_data=iter(vbenc),
            validation_steps=validation_steps,
            steps_per_epoch=steps_per_epoch,
            epochs=epochs,
