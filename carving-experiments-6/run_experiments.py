...
categories = [x+'-'+y for x in ['pdf','png', 'jpg'] for y in ['ini', 'mid', 'end']]
...
def xs_ys_from_filenames(filenames):
    ...
    for i,f in enumerate(filenames):
        cat = utils.load.category_from_extension(f)
        count = count_sectors(f)
        sector = np.random.randint(0,count)
        sector = np.random.choice([0,count-1,sector], p=[0.1,0.4,0.5])
        suffix = 'mid'
        if sector == count:
            suffix = 'end'
        if sector == 0:
            suffix = 'ini'
        cat = cat + '-' + suffix
        x = get_sector(f,sector,fill_random=False)
